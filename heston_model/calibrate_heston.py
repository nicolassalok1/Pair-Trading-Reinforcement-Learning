"""
Heston calibration utilities (Carr–Madan) made importable for RL features.

Original logic preserved:
- read CSV with columns S0, K, T, C_mkt, iv_market
- filter valid points
- Adam-like optimisation on unconstrained parameters

Public helpers for RL:
- calibrate_heston_from_df
- heston_params_to_features
- get_heston_features_from_csv
- get_heston_features_from_market_df
"""
...
try:
    ...
    from .heston_torch import HestonParams, carr_madan_call_torch  # type: ignore
except Exception:
    try:
        # Fallback to external package on PYTHONPATH
        from Heston.heston_torch import HestonParams, carr_madan_call_torch  # type: ignore
    except Exception as exc:  # pragma: no cover - defensive
        raise ImportError(
            "heston_torch module not found. Ensure Heston.heston_torch or heston_model.heston_torch "
...
def calibrate_heston_from_df(
    ...
    device: torch.device | None = None,
    progress: bool = False,
) -> tuple[HestonParams, dict, pd.DataFrame]:
    ...
    metrics = {
        ...
        "n_points": int(len(df_used)),
    }
    return params, metrics, df_used


def heston_params_to_features(params: HestonParams) -> np.ndarray:
    """
    Convertit les paramètres Heston en vecteur de features pour RL.
    Retourne [kappa, theta, sigma, rho, v0] (shape (5,)).
    """
    arr = np.array(
        [
            float(params.kappa.detach().cpu()),
            float(params.theta.detach().cpu()),
            float(params.sigma.detach().cpu()),
            float(params.rho.detach().cpu()),
            float(params.v0.detach().cpu()),
        ],
        dtype=np.float64,
    )
    return arr
...
def get_heston_features_from_csv(
    csv_path: Path,
    r: float,
    q: float,
    max_iters: int = DEFAULT_ITERS,
    ...
    """
    Lit un CSV (S0, K, T, C_mkt, iv_market), calibre Heston, et renvoie un vecteur de features (5,) + métriques.
    """
    df = pd.read_csv(csv_path)
    params, metrics, _ = calibrate_heston_from_df(
        df=df,
        r=r,
        q=q,
        max_iters=max_iters,
    ...
    features = heston_params_to_features(params)
    return features, metrics


def get_heston_features_from_market_df(
    ...
    """
    Pareil que get_heston_features_from_csv mais sans lecture disque.
    """
    params, metrics, _ = calibrate_heston_from_df(
        df=df_options,
        r=r,
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    default_csv = here / "heston_dummy_calls.csv"
    if not default_csv.exists():
        default_csv = here.parent / "heston_dummy_calls.csv"
